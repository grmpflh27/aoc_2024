...
fn main() {
    ...
    let mut print_rules: HashMap<i32, Node> = HashMap::new();

    let instrs_str = &read_lines(&test_file_path);
    for line in instrs_str.lines() {
        let instr: Vec<_> = line.split('|').map(|i| i.parse::<i32>().unwrap()).collect();
        let mut node: Node = Node {
            page: instr[0],
            pages_after: vec![instr[1]],
        };
        if !print_rules.contains_key(&node.page) {
            print_rules.insert(node.page, node);
        } else {
            let mut existing = &print_rules[&node.page];
